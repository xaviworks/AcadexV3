...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex flex-wrap gap-2">
            <a href="{{ $buildRoute('vpaa.gradingConfiguration.departments.show', ['department' => $department->id]) }}" class="btn btn-outline-secondary btn-sm">
                <i class="bi bi-arrow-left me-1"></i>Back to Department
            </a>
            <a href="{{ $buildRoute('vpaa.gradingConfiguration.courses.edit', ['department' => $department->id, 'course' => $course->id]) }}" class="btn btn-success btn-sm">
                <i class="bi bi-pencil-square me-1"></i>{{ $needsCourseFormula ? 'Create Course Formula' : 'Edit Course Formula' }}
            </a>
        </div>
    </div>
    <div class="d-flex justify-content-end mb-3">
        <form method="GET" action="{{ route('vpaa.gradingConfiguration.courses.show', ['department' => $department->id, 'course' => $course->id]) }}" class="d-flex align-items-center gap-2 flex-wrap">
        ...
        <div class="row g-4" id="subject-wildcards">
            @foreach($subjectSummaries as $summary)
                ...
                <div class="col-12 col-sm-6 col-lg-4 col-xl-3">
                    <a href="{{ $buildRoute('vpaa.gradingConfiguration.subjects.show', ['subject' => $subject->id]) }}" class="text-decoration-none text-reset">
                        <div class="wildcard-card card h-100 border-0 shadow-lg rounded-4 overflow-hidden cursor-pointer transition-transform-shadow" data-status="{{ $summary['status'] }}" data-url="{{ $buildRoute('vpaa.gradingConfiguration.subjects.show', ['subject' => $subject->id]) }}">
                            {{-- Top header --}}
...
{{-- JavaScript moved to: resources/js/pages/vpaa/grading-configuration-course.js --}}
