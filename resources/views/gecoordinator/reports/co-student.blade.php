...
<div class="container-fluid px-4 py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            <a href="{{ route('gecoordinator.reports.co-student') }}" class="btn btn-outline-secondary rounded-pill">
                        ...
                        <tr class="table-success fw-bold">
                            ...
                            @foreach($coColumnsByTerm as $coCode)
                                ...
                                <td class="text-center">
                                    @if($val)
                                        @php($threshold = (int) ($val['target_percentage'] ?? 75))
                                        <span class="badge {{ $val['percent'] >= $threshold ? 'bg-success' : 'bg-danger' }} px-3 py-2 rounded-pill text-white">
                                            {{ number_format($val['percent'], 2) }}%
                                        </span>
                                        <div><small class="text-muted">{{ $val['raw'] }}/{{ $val['max'] }} | target {{ $threshold }}%</small></div>
                                    @else
