...
const instructorUpdatesChannel =
  typeof BroadcastChannel !== 'undefined' ? new BroadcastChannel('ac-instructor-updates') : null;
    ...
    .then((resp) => {
      if (!resp.ok)
        return resp
          .json()
          .then((err) => {
            throw new Error(err.message || 'Failed to load instructors');
          })
          .catch(() => {
...
export function openViewInstructorsModal(subjectId, subjectName) {
  ...
  fetch(`/gecoordinator/subjects/${subjectId}/instructors`)
    ...
    .then((data) => {
      ...
      if (countEl) {
        countEl.textContent =
          count === 0 ? 'No instructors assigned' : `${count} instructor${count !== 1 ? 's' : ''} assigned`;
      }
    ...
    .then(([assignedResp, availableResp]) => {
      if (!assignedResp.ok) {
        return assignedResp
          .json()
          .then((err) => {
            throw new Error(err.message || 'Failed to load assigned instructors');
          })
          .catch(() => {
      ...
      if (!availableResp.ok) {
        return availableResp
          .json()
          .then((err) => {
            throw new Error(err.message || 'Failed to load available instructors');
          })
          .catch(() => {
...
export function assignMultipleInstructors(subjectId, instructorIds, button) {
  ...
  if (button) {
    ...
    button.innerHTML =
      '<span class="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true"></span>Assigning...';
  }
  ...
  Promise.all(
    ...
    .then((results) => {
      ...
      setTimeout(() => {
        openInstructorListModal(
          subjectId,
          document.getElementById('instructorListSubjectName')?.textContent || '',
          'view'
        );
...
export function initAssignSubjectsPage() {
  ...
  if (confirmUnassignBtn) {
    confirmUnassignBtn.addEventListener('click', function () {
      ...
      Promise.all(
        currentUnassignInstructorIds.map((id) => {
