...
export function initGradesFormulaWildcards() {
  ...
  const createFormulaModalInstance =
    createFormulaModal && window.bootstrap?.Modal
      ? window.bootstrap.Modal.getOrCreateInstance(createFormulaModal)
      : null;
...
function initStructureTemplateManagement(config) {
  ...
  const bootstrapModalInstance =
    templatePasswordModal && window.bootstrap?.Modal ? new window.bootstrap.Modal(templatePasswordModal) : null;
  const deleteTemplateModalInstance =
    deleteTemplateModal && window.bootstrap?.Modal
      ? window.bootstrap.Modal.getOrCreateInstance(deleteTemplateModal)
      : null;
  ...
  function applyTemplateMode(mode, templateData = null, options = {}) {
    ...
    if (mode === 'edit' && templateData) {
      if (templateUpdateActionPattern) {
        createTemplateForm.action = templateUpdateActionPattern.replace(
          templateUpdatePlaceholder,
          templateModeState.editingId ?? ''
        );
      }
  ...
  function syncMainComponentMaxState(mainComponentId) {
    ...
    const mainComponent = document.querySelector(
      `.component-item[data-component-id="${mainComponentId}"][data-is-main="true"]`
    );
    if (!mainComponent) {
      return;
    }
  ...
  function restoreTemplateComponents(oldComponents) {
    ...
    const subEntries = entries.filter(
      ([, component]) => !isMainComponent(component) && resolveComponentValue(component, 'parent_id', null) !== null
    );
  ...
  if (templateDeleteReopenId && deleteTemplateModalInstance) {
    ...
    if (deleteTemplateForm && deleteTemplateActionPattern) {
      deleteTemplateForm.action = deleteTemplateActionPattern.replace(
        templateUpdatePlaceholder,
        templateDeleteReopenId
      );
    }
