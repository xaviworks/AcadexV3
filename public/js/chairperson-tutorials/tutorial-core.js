...
(function() {
    ...
    window.ChairpersonTutorial = {
        ...
        getCurrentPageId: function() {
            ...
            if (path.includes('/chairperson/structure-templates')) {
                if (path.includes('/create')) {
                    return 'chairperson-structure-templates-create';
                }
                // Check for show page (has ID in path)
                if (path.match(/\/structure-templates\/\d+/)) {
                    return 'chairperson-structure-templates-show';
                }
                return 'chairperson-structure-templates';
            }
            
            // Grades - Multi-step wizard
            if (path.includes('/chairperson/grades')) {
                ...
                if (urlParams.get('course_id')) {
                    return 'chairperson-grades-instructors';
                }
            ...
            if (path.includes('/chairperson/assign-subjects')) {
                return 'chairperson-assign-subjects';
            }
            ...
            if (path.includes('/chairperson/instructors')) {
                ...
                if (hash === '#inactive' || document.querySelector('#inactive-instructors-tab.active')) {
                    return 'chairperson-instructors-inactive';
                }
                if (hash === '#pending' || document.querySelector('#pending-approvals-tab.active')) {
                    return 'chairperson-instructors-pending';
                }
                return 'chairperson-instructors';
            }
            
            // Dashboard - check for dashboard path or root with role detection
            if (path === '/dashboard' || path === '/' || path.includes('/chairperson/dashboard')) {
                // Check if we're on the Chairperson dashboard by looking for specific elements
                const chairTitle = document.querySelector('h2.fw-bold');
                if (chairTitle && (chairTitle.textContent.includes('Course Chair') || chairTitle.textContent.includes('Chairperson'))) {
        ...
        showNoDataStep: function(step) {
            ...
            if (!targetEl) {
                targetEl = this.findTarget('table thead, .card-header, .container-fluid h1, .page-title h1');
            }
            ...
            if (check.noAddButton) {
                content += `There are no ${entityName} to display yet. ${entityName.charAt(0).toUpperCase() + entityName.slice(1)} will appear here when instructors register or are added.`;
            } else {
