/**
 * Instructor Tutorial System - Core Manager
...
(function() {
    ...
    window.InstructorTutorial = {
        ...
        STORAGE_PREFIX: 'acadex_instructor_tutorial_',
        ...
        getCurrentPageId: function() {
            ...
            if (path.includes('/instructor/course-outcome-attainment')) {
                return 'instructor-course-outcome-attainment';
            }

            // Course Outcomes
            if (path.includes('/instructor/course_outcomes') || path.includes('/instructor/course-outcomes')) {
                return 'instructor-course-outcomes';
            }

            // Activities - Create (more specific, check first)
            if (path.includes('/instructor/activities/create')) {
                return 'instructor-activities-create';
            }

            // Activities - Index
            if (path.includes('/instructor/activities')) {
                return 'instructor-activities';
            }

            // Manage Students
            if (path.includes('/instructor/manage-students') || path.includes('/instructor') && path.includes('students')) {
                return 'instructor-manage-students';
            }

            // Manage Grades
            if (path.includes('/instructor/manage-grades') || (path.includes('/instructor/grades') && !path.includes('/scores'))) {
                return 'instructor-manage-grades';
            }

            // Scores/Final Grades
            if (path.includes('/instructor/scores') || path.includes('/instructor/final-grades')) {
                return 'instructor-scores';
            }

            // Instructor Dashboard (check last since it's most general)
            if (path === '/dashboard' || (path.includes('/dashboard') && document.querySelector('.bi-cloud-sun-fill, .bi-brightness-high-fill, .bi-cloud-moon-fill'))) {
        ...
        createOverlayElements: function() {
            ...
            this.nextHint = document.createElement('div');
            this.nextHint.className = 'tutorial-next-hint';
            this.nextHint.id = 'tutorial-next-hint';
            this.nextHint.innerHTML = `
                <div class="tutorial-hint-pulse"></div>
                <div class="tutorial-hint-label">Next</div>
            `;
            document.body.appendChild(this.nextHint);
        },
        ...
        showNoDataStep: function(step) {
            ...
            let content = `The table is currently empty. To see more features of this page, please add some ${entityName} first, then restart this tutorial.`;
        ...
        showStep: function(stepIndex) {
            ...
            this.showNextStepHint(stepIndex);
        ...
        /**
         * Show hint for the next step's target element
         */
        showNextStepHint: function(currentStepIndex) {
            if (!this.currentTutorial || !this.nextHint) return;

            const steps = this.currentTutorial.steps;

            // Don't show hint on last step
            if (currentStepIndex >= steps.length - 1) {
                this.hideNextStepHint();
                return;
            }

            // Find next step with a visible target
            let nextStepIndex = currentStepIndex + 1;
            let nextTargetEl = null;

            while (nextStepIndex < steps.length && !nextTargetEl) {
                const nextStep = steps[nextStepIndex];

                // Skip if next step requires data but no data exists
                if (nextStep.requiresData && !this.hasTableData()) {
                    nextStepIndex++;
                    continue;
                }

                nextTargetEl = this.findTarget(nextStep.target);

                // If optional and not found, try next
                if (!nextTargetEl && nextStep.optional) {
                    nextStepIndex++;
                } else {
                    break;
                }
            ...
            // If we found a next target, show the hint
            if (nextTargetEl && this.isVisible(nextTargetEl)) {
                const rect = nextTargetEl.getBoundingClientRect();

                // Position hint at the center of the next target
                this.nextHint.style.top = (rect.top + window.scrollY + rect.height / 2 - 20) + 'px';
                this.nextHint.style.left = (rect.left + window.scrollX + rect.width / 2 - 20) + 'px';
                this.nextHint.classList.add('active');
            } else {
                this.hideNextStepHint();
            }
        },
        ...
        /**
         * Hide the next step hint
         */
        hideNextStepHint: function() {
            if (this.nextHint) {
                this.nextHint.classList.remove('active');
            }
        },
                ...
                if (sel.includes(':contains')) {
                    ...
                    if (match) {
                        ...
                        for (const el of elements) {
                            if (el.textContent.includes(text) && this.isVisible(el)) {
                                return el;
                            }
                        }
                    }
        ...
        showCompletion: function() {
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    ...
                    text: 'You\'ve completed the tutorial. You can restart it anytime using the help button.',
                    icon: 'success',
                    confirmButtonColor: '#198754',
